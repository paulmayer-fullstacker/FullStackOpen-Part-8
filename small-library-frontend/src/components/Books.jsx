...
const Books = (props) => {
  ...
  const filteredBookList = useQuery(ALL_BOOKS, {
    // Argument variables: If genreFilter is 'all genres', send null so the backend returns everything. Else send the genreFilter. Null represents NO Filters.
  ...
  useEffect(() => {
    if (props.show) {
      // refetch() ensures that if another user added a book while we are viewing the Authors page, the Books list is updated when we switch back.
  ...
  const allGenres = [
    ...
    ...new Set(
      unfilteredBooksList.data.allBooks.flatMap((book) => book.genres),
    ),
  ];
    ...
    <div>
      ...
      <table>
        <tbody>
          ...
          {/* Iterate (map) over the 'books' array. For every book object, return a new table row (tr). */}
          {filteredBooks.map(
            (
              book, // Key attribute required to identify table row. Use the unique 'book id' from the database to track this specific row. Using the key, React can just rerender a single row (that has changed) without rerendering the entire component.
            ) => (
              <tr key={book.id}>
                <td>{book.title}</td>
